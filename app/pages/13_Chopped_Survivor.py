...
cur_week = st.number_input("Current week", min_value=1, max_value=18, value=1,
                           step=1, key="cs_week")
...
for i, (col, label) in enumerate(zip(entries, ("Your entry", "Your wife's entry"))):
    with col:
        ...
        used = st.multiselect("Teams already used", all_teams, default=[],
                              key=f"cs_used_{i}")
        plan_df, opts = _plan(frozenset(used), int(cur_week), _TW_KEY)
        if opts.empty:
        ...
        st.markdown(
            ...
            f'{"vs" if best["is_home"] else "@"} {best["opponent"]}</div></div>',
            unsafe_allow_html=True)
...
if len(recs) == 2 and len(set(recs.values())) == 1:
    st.info(
        f"Both entries land on **{list(recs.values())[0]}**. Only one of you can win the pot, "
        "so playing the same team means you are eliminated together. Take the "
...
st.markdown(f"""
**1 · Diverge on purpose.** Only one entry can take the pot. Picking the same team
every week means you are eliminated on the same Sunday and split nothing. Week 1 is
the cheapest place to split — take the 0.00-cost pick on one entry and the next
option on the other, then let the two plans drift apart naturally.

**2 · Spend the cheap wins, hoard the expensive ones.** Almost every good week comes
...
**3 · Work backwards from Week {_worst_week}.** It is the thinnest week left: the best
...
**5 · The tiebreaker rewards the same discipline.** Ties are broken by the combined wins of
...
**6 · Ties on the field count as wins.** A pick only fails if your team actually loses,
